...
Data sortedArray::find(const myVector<char>& key) const {
...
int sortedArray::findOnIndex(const myVector<char>& key) const {
	if (key.size() == 0) throw std::runtime_error("Invalid key");
	int low = 0, high = data.size() - 1, mid;
	while (low <= high) {
		mid = low + (high - low) / 2;
...
void sortedArray::ins(const Data& _data) {
...
void sortedArray::del(const myVector<char>& key) {
